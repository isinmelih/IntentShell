...
title IntentShell CLI (Debug / Headless Mode)
...
title IntentShell CLI (Debug / Headless Mode)
echo.
echo [INFO] Starting IntentShell CLI (Debug / Headless Mode)...
...
if errorlevel 1 (
    ...
    echo   --- If you arrived here after seeing the suggestion in the Start-IntentShell.bat, the problem isn't with the UI, but with the program's main files.
    echo   --- SUGGESTION: Please download and run the project again from GitHub.
    echo   --- If the above suggestion doesn't work, check the accuracy of the following file paths:
    echo   --- PowerShell7: C:\Program Files\PowerShell\7\pwsh.exe
    echo   --- Python(user-specific): C:\Users\[YOUR USER NAME]\AppData\Local\Python\[also check for Programs folder]\pythoncore-3.1x-xx(Python 3.10+ Required)
    echo   --- Python(For all users): C:\C:\Program Files\Python3xx\python.exe
    echo   --- Python(From Microsof Store): C:\Users\[YOUR USER NAME]\AppData\Local\Microsoft\WindowsApps\python.exe
    echo   --- Python(Multiple Versions): C:\Windows\py.exe
    echo   --- (CHECK the CORRECT PATH according to how you installed Python.)
    echo   --- [Start-IntentShell.bat and Start-IntentShell-CLI.bat install Python FOR ALL USERS.]
    echo   --- PowerShell7: C:\Program Files\PowerShell\7\pwsh.exe
    echo   --- If the above file paths are valid and the program still doesn't work, run DeviceDebug.bat. The error will be displayed there in full detail.
    echo.
    ...
    echo [INFO] main.py finished normally.
)
